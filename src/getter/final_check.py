...
FINAL_CHECK_ADDR = range(0x2A3, 0x2A9)
...
def final_check(diction):
    ...
    # check endurance test cycle
    if(diction["Edurance_Pass"] < 149):
        vpd.vpd_close()
        raise error.EDURA_PTIME_FAIL
    if(diction["Edurance_Fail"] > 1):
        vpd.vpd_close()
        raise error.EDURA_FTIME_FAIL
